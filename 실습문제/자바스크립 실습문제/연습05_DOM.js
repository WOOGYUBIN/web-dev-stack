...
function prinText() {
  result1.innerHTML(또는 textContent써도 됨) = "안녕하세요";
} */
...
function printInputValue() {
  //input값을 가지고 출력해야함 (쪼개서 풀기!)
  // 1 input값(customer)을 가지고 오기
  console.log(customer.value);
  // 2 가지고 와서 result2에 출력해야 함
...
function changeColor() {
  // 선택한 divTest에 배경색을 변경해야됨
  divTest.style.backgroundColor = "red"; //backgroundColor 대신에 background라고 해도 됨
}
...
function stringSplit() {
  ...
  console.log(la.textContent); // console.log(la.innerHTML); 이거 써도됨
...
function addToCart() {
  ...
  if (!isNaN(Number(price.value))); //NaN
  total.innerHTML = sum;
  console.log(sum);
