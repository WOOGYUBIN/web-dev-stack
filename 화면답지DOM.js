// console로 확인후 필요한거는 변수에 담기
const currentTime = () => {
  const weeks = ["일", "월", "화", "수", "목", "금", "토"];
  const today = new Date(); //내장객체 파일에 있음
  console.log(today);
  console.log(today.getFullYear()); //년
  console.log(today.getMonth() + 1); // 월 (0 : 1월, 1 : 2월이라서 정확한 월은 +1해야됨)
  console.log(today.getDate()); //일 단위는 getDate
  console.log(today.getDay()); //요일 (0: 일요일, 1: 월요일)
  // console.log(weeks[3]); 수요일임
...
const randomBackground = () => {
  ...
  const randomBackgroundbackground = () => {
    const body = document.body;
...
const count = () => {
  const last = new Date("2026-01-01 00:00:00"); //목표히는 날찌
  const now = new Date();
  // console.log(Math.floor((last - now) / 1000);
  const second = Math.floor((last - now) / 1000); // 1초 = 밀리세컨드/1000
  const minute = Math.floor(second / 60); // 60ch = 1분
