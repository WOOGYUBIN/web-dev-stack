/* 
    ...
    - SELECT는 데이터 조회를 의미하여 일부에서는 DQL(Date Query Language)로 분류하기도 함
...
SELECT NAME, AGE, HOBBY FROM USER_INFO; -- 오라클 (대문자로 씀) 
...
SELECT NAME, AGE, '살' FROM USER_INFO;
SELECT NAME, AGE || '살' AS 나이 FROM USER_INFO;

-- USER_INFO에서 이름(NAME)의 MBTI는 XXXX입니다 조회
SELECT NAME||'의 MBTI는 '|| MBTI || '입니다' AS "MBTI 소개" FROM USER_INFO;

/*
    DISTINCT
    - 컬럼에 중복된 값들을 한번씩만 표시하고자 할 때 
    - 주의사항 : DISTINCT는 SELECT절에서 한 번만 기술 가능함 
*/
...
-- 테이블 : EMPLOYEE
-- 급여(SALARY)가 350만원이상 600만원 이하를 받는 사원들 조회 
...
/*
    ...
    '_문자' : 앞에 1글자 + 문자 
    '__문자' : 앞에 2글자 + 문자 
    '_문자_' : (문자기준으로) 앞뒤에 1글자씩 + 문자
    
    '%' : 0글자 이상
    '문자%' : 문자로 시작
    '%문자' : 문자로 끝남 
    '%문자%' : (앞뒤 상관없이) 문자 포함 (키워드 검색할때 사용) 
*/
-- 테이블 : USER_INFO
-- 이름이 '은'으로 끝나는 경우 조회
SELECT * FROM USER_INFO WHERE NAME LIKE '%은'; -- '__은'도 됨
-- 성이 '성'씨인 경우 조회 
SELECT * FROM USER_INFO WHERE NAME LIKE '성%';
-- MBTI가 'E'면서 'T'인 경우 조회
SELECT * FROM USER_INFO WHERE MBTI LIKE '%E%T%'; -- 'E_T_'이렇게 써도됨
...
-- EMPLOYEE에서 직급코드(JOB_CODE)가 J7이거나 J2 사원들 중 급여가 200만원인 사원들 조회
-- IN 연산자 사용/ OR 연산자 두개다 이용해서 조회
SELECT *
FROM EMPLOYEE
...
--괄호를 안하면 AND가 계산이 먼저 되고 그 다음에 OR이 계산됨 괄호를 쓰면 괄호안을 제일먼저 처리해줌 
SELECT *
FROM EMPLOYEE
...
SELECT * 
FROM USER_INFO 
ORDER BY AGE; --AGE ASC 이렇게 쓰는거랑 같음
...
FROM USER_INFO 
ORDER BY 4 ASC, 2 DESC; --4번째 ASC정렬, 2번째 DESC정렬 이라는 뜻

-- 조건은 MBTI가 I인 분들 조회 
SELECT NAME, AGE, GENDER, MBTI 
FROM USER_INFO 
ORDER BY MBTI DESC NULLS LAST; --NULL들을 맨끝으로 보내고 정렬 

-- EMPLOYEE에서 전체 사원의 사원명, 보너스 조회 
-- 보너스 기준 오름차순 정렬 
SELECT EMP_NAME, BONUS 
FROM EMPLOYEE
-- ORDER BY BONUS NULLS FIRST; 오름차순인 경우는 NULL이 맨뒤에 있음 
