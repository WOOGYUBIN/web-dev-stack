...
*/
SELECT 
INSTR('AABAACAABBAA', 'B', -1, 2), -- -1이라 기준이 뒤이지만, 뒤에서부터 2번째에 있는 B를 찾음, 순서는 앞에서부터 계산
INSTR('AABAACAABBAA', 'D')
FROM DUAL;

-- USER_INFO에서 각 전화번호(CONTACT)에서 앞에서부터 5가 들어간 위치 조회 
SELECT CONTACT,
INSTR (CONTACT, 5) -- -이것도 1개로 취급함 그래서 5가 5번째 있다는 뜻
FROM USER_INFO;
...
*/
SELECT 
LPAD('HELLO', 10),  --왼쪽앞에가 5글자가 비어져있음 
LPAD('HELLO', 10, 'A'), --덧붙이고자 하는 문자는 왼쪽부터 채움(LPAD에서 L은 Light)
...
RTRIM('46548518KH54568','0123456789') --오른쪽에서 0123456789에 해당하는 숫자들 제거 
FROM DUAL;
...
*/
SELECT 
TRIM(LEADING 'Z' FROM 'ZZZKHZZZ'), --LTRIM 이랑 같음
TRIM(TRAILING 'Z' FROM'ZZZKHZZZ'), --RTRIM 이랑 같음
TRIM(BOTH 'Z' FROM 'ZZZKHZZZ'), -- 양쪽 모두 제거 
TRIM('       KH       ') -- 앞뒤 공백이 있는 경우 공백을 전부 제거 해줌 
FROM DUAL;
...
SELECT CONTACT,
SUBSTR(CONTACT, 5, 4), -- = SUBSTR(CONTACT, INSTR(CONTACT, '-') + 1, 4), 같음
INSTR(CONTACT, '-') + 1 -- 5번째를 의미함
FROM USER_INFO;
...
ROUND(123.456, 1), --123.5 (1이라는게 소수점 첫째자리를 의미함) 
ROUND(123.456, -2) --100 
FROM DUAL;
/*
    TRUNC(숫자,위치)
...
SELECT 
TRUNC(123.952), -- 123
TRUNC(123.952, 1)  -- 123.9 (소수점 첫째자리까지 나타남)
FROM DUAL;
...
--날짜 포맷 변경 
ALTER SESSION SET NLS_DATE_FORMAT = 'YYYY-MM-DD HH:MI:SS'; --분: MI  --월: MM
...
*/
SELECT MONTHS_BETWEEN('20250704', SYSDATE) --지금(6월4일임) 7월4일까지는 딱 1개월 남았다는 의미 
FROM DUAL;

SELECT FLOOR(MONTHS_BETWEEN('20251216', SYSDATE)) 
FROM DUAL;

--USER_INFO에서 몇개월생인지 조회 (BIRTHDATE활용)
...
*/
SELECT ADD_MONTHS(SYSDATE, 2) FROM DUAL; --여기서 2는 2개월후라는 뜻임
...
*/
SELECT TO_CHAR(1234, 'L99,999') --$1,234   L은 현재 설정된 화폐 단위에 따라서 바뀜 
FROM DUAL;
ALTER SESSION SET NLS_CURRENCY= '$';  -- 역슬래시 표시이기도 하지만 원을 나타내는 표시임  
...
-- 날짜 -> 문자
SELECT
TO_CHAR(SYSDATE, 'YYYY'), --년 : YYYY, YY, RR, RRRR, YEAR 
TO_CHAR(SYSDATE, 'MM'), --월 : MM, MON, MONTH, RM
TO_CHAR(SYSDATE, 'DD'), --일 : D- 주 기준(일요일이 1임, ...) DD -월 단위임 , DDD - 년 기준  
...
-- 문자 -> 날짜
SELECT TO_DATE('20250604') FROM DUAL;
SELECT TO_DATE('20250604', 'YYYY-MM-DD HH24:MI:SS') FROM DUAL;
/*
    TO_NUMBER(문자,포맷)
    -문자형 데이터를 숫자 타입으로 변환 
*/
SELECT '100000' + '550000' FROM DUAL;
SELECT TO_NUMBER('100,000', '999,999') + TO_NUMBER('550,000', '999,999') + TO_NUMBER('550,000', '999,999')
FROM DUAL;
...
SELECT 
SUBSTR(EMP_NO, 8, 1), --8번째 해당하는거 1개가져옴 
...
CASE WHEN SALARY > 5000000 THEN '1등급' 
     WHEN SALARY > 3500000 THEN '2등급' -- WHEN SALARY BETWEEN 3500000 AND 5000000 THEN '2등급'
...
*/
-- 전체 USER_INFO 전체 사람 수 조회 
SELECT 
COUNT(*) -- COUNT(*)랑 같음 COUNT(USER_ID), COUNT(NAME) 등등 가능
FROM USER_INFO;
