...
-- USER_TAB_COLUMNS : 사용자가 가지고 있는 테이블드르이 상의 모든 컬럼의 전반적인 구조를 확인할 수 있는 시스템 테이블 
...
VALUES(3,'user03', 'pass03', '곽병현', 'o', SYSDATE); 
...
/*
    ...
    * 삭제옵션: 부모테이블의 데이터 삭제 시 그 데이터를 사용하고 있는 자식테이블의 값을 어떻게 처리할 건지  
    - ON DELETE RESTRICTED (기본값) : 자식데이터로 쓰이는 부모 데이터는 삭제 아예 안되게끔 
...
/*
    DEFAULT 기본값 
    - 제약조건 아님! 
    - 컬럼을 선정하지 않고 INSERT 시 NULL이 아닌 기본값을 INSERT 하고자 할때 세팅해둘 수 있는 값
*/
DROP TABLE MEMBER;
CREATE TABLE MEMBER(
    MEM_NO NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY, 
...
VALUES('오픈도어북스','010-1234-5678');
INSERT INTO PUBLISHER (PUB_NAME, PHONE)
VALUES('모티브','010-1111-2222');
...
VALUES('시원한 바다', '여름이가',1);
INSERT INTO BOOK (BK_TITLE, BK_AUTHOR, PUB_NO)
VALUES('누구도 대신 살아주지 않는다', '브라이언 트레이시',2);
INSERT INTO BOOK (BK_TITLE, BK_AUTHOR, PUB_NO)
...
FROM RENT R  --R은 RENT라는 테이블에 별칭을 붙인거임 
...
*/
--DESC: 기존 데이터 타입을 어떤 식으로 지정했는지에 대한 내용을 볼 수 있음
DESC USER_INFO; 
...
/*
    2. 제약조건
    2-1. 제약조건 추가 
    PRIMARY KEY : ADD [CONSTRAINT 제약조건명] 
...
*/
ALTER TABLE DEPT_COPY RENAME CONSTRAINT SYS_C008336 TO DEPT_ID_PK;
